...
/*		Wire.beginTransmission(I2CAddress);
		...
		{  */
			...
			//{
				int dane3 = wiringPiI2CReadReg8(fd,I2CAddress);

				c = dane3; // receive a byte as character
				*outputPointer = c;
				outputPointer++;
				i++;
			//}
...
printf(" I2C modul ADXL 345");
	wiringPiI2CWrite(fd, offset);

	//	if( Wire.endTransmission() != 0 )
	//	{
	//		returnError = IMU_HW_ERROR;
	//	}
	int dane2 = wiringPiI2CReadReg8(fd,numBytes);
	//	Wire.requestFrom(I2CAddress, numBytes);
	//	while ( Wire.available() ) // slave may send less than requested
	//	{
			result = dane2;// Wire.read(); // receive a byte as a proper uint8_t
	//	}
		break;
...
printf(" I2C modul ADXL 345");
	wiringPiI2CWrite(fd, offset);
	wiringPiI2CWrite(fd, dataToWrite);
...
{
	//status_t returnError = wiringPiI2CWrite(LSM6DS3_ACC_GYRO_RAM_ACCESS, 0x00); //writeRegister( LSM6DS3_ACC_GYRO_RAM_ACCESS, 0x00 );
...
status_t LSM6DS3::begin()
{
	...
	wiringPiI2CWrite(LSM6DS3_ACC_GYRO_CTRL1_XL, dataToWrite);
	//writeRegister(LSM6DS3_ACC_GYRO_CTRL1_XL, dataToWrite);

	//Set the ODR bit
	//wiringPiI2CReadReg8(LSM6DS3_ACC_GYRO_CTRL4_C,&dataToWrite);
	dataToWrite= wiringPiI2CReadReg8(fd ,LSM6DS3_ACC_GYRO_CTRL4_C);
	...
	wiringPiI2CWrite(LSM6DS3_ACC_GYRO_CTRL4_C, dataToWrite);
	//Setup the gyroscope**********************************************
	dataToWrite = 0; //Start Fresh!
...
wiringPiI2CWrite(LSM6DS3_ACC_GYRO_CTRL2_G, dataToWrite);
	...
	result= wiringPiI2CReadReg8(fd ,LSM6DS3_ACC_GYRO_WHO_AM_I_REG);
	return returnError;
}
...
{
	int16_t output;
	output= wiringPiI2CReadReg16(fd ,LSM6DS3_ACC_GYRO_OUTX_L_XL);//readRegisterInt16( &output, LSM6DS3_ACC_GYRO_OUTX_L_XL );
